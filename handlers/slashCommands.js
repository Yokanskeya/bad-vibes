			...
			client.on("ready", () => {
				if(config.loadSlashsGlobal){
					...
					.then(slashCommandsData => {
						...
						console.logger(`${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.green} Loaded for all: ${`All possible Guilds`.underline}`.brightGreen); 
					...
					client.guilds.cache.map(g => g).forEach(async (guild) => {
						try{
							...
							.then(slashCommandsData => {
								client.slashCommandsData = slashCommandsData;
								console.logger(`${slashCommandsData.size} slashCommands ${`(With ${slashCommandsData.map(d => d.options).flat().length} Subcommands)`.green} Loaded for: ${`${guild.name}`.underline}`.brightGreen); 
							}).catch((e)=>{});
						} catch (e){
