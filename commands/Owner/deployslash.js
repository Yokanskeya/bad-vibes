...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    if (loadSlashsGlobal) {
      let themsg = await message.reply(`<a:Loading:927444585469313076> **Attempting to set the Global Slash Commands in \`${client.guilds.cache.size} Guilds\`...**`)
      ...
      let guild = client.guilds.cache.get(guildId);
      let themsg = await message.reply(`<a:Loading:927444585469313076> **Attempting to set the GUILD Slash Commands in \`${guild.name}\`...**`)
