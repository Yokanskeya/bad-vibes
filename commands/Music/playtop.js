...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix, player, es, ls) => {
    ...
    if (args.join("").includes("soundcloud")) {
      message.reply({
        embeds: [ new MessageEmbed().setColor(es.color)
          .setTitle(eval(client.la[ls]["cmds"]["music"]["playtop"]["soundcloud"]))
      ...
      message.reply({
        embeds: [
          ...
          .setTitle(eval(client.la[ls]["cmds"]["music"]["playtop"]["spotify"]))
          .setDescription(`\`\`\`${String(args.join(" ")).substr(0, 2000)}\`\`\``)
        ]
      })
      ...
      message.reply({
        embeds: [
          new MessageEmbed().setColor(es.color)
          .setTitle(eval(client.la[ls]["cmds"]["music"]["playtop"]["apple"]))
          .setDescription(`\`\`\`${String(args.join(" ")).substr(0, 2000)}\`\`\``)
        ]
      })
      playermanager(client, message, args, `playtop:raw`);
      ...
      message.reply({
        embeds: [
          new MessageEmbed().setColor(es.color)
          .setTitle(eval(client.la[ls]["cmds"]["music"]["playtop"]["youtube"]))
          .setDescription(`\`\`\`${String(args.join(" ")).substr(0, 2000)}\`\`\``)
        ]
      })
      //play from YOUTUBE
